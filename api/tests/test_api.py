...
class FishIntegrationTest(APITestCase):
    def setUp(self):
        ...
        self.technique = Technique.objects.create(
            description="Use live bait on a splitshot."
        )
    ...
    def test_full_fish_workflow(self):
        ...
        response = self.client.post(
            "/api/fish/",
            {
                "species": "Redfish",
                "trends": [
                    "Shallow water during tide changes."
                ],  # List of strings for trends
                "techniques": [
                    "Use live or dead bait."
                ],  # List of strings for techniques
                "tutorial_video": "https://example.com/redfish-tutorial",
                "bait": [
                    {
                        "name": "Shrimp",
                        "properties": "Small, Crustacean",
                        "notes": "Attracts Redfish",
                    }
                ],
                "food": [
                    {
                        "name": "Crustaceans",
                        "properties": "Various small species",
                        "notes": "Common food for Redfish",
                    }
                ],
            },
            format="json",
    ...
    def test_get_fish_list(self):
        ...
        self.assertEqual(
            fish_data["tutorial_video"], "https://example.com/snook-tutorial"
        )
